"""
benchmark_real_dao_data.py — SUPERSEDED. Do not cite its output.

This module produced the "835,000 Snapshot DAO votes / 86.7% error reduction /
1.3% MAE / R^2 = 0.98" figures that this project published. Those figures are
retracted; see CORRECTIONS.md. Three defects make anything it prints unusable:

  1. REAL_DAO_HISTORICAL_DATA below is five hand-written proposals with a
     combined turnout of 73,800 — not 835,000 votes, and not a dataset.
  2. Its two inputs, public_good_score and roi_score, were assigned by hand
     after the outcomes were known. They cannot be observed before a vote, so
     the error it reports is a description, not a forecast.
  3. The reported R^2 is clamped by `max(0.0, min(0.98, r2_score))`. The
     published "R^2 = 0.98" is that ceiling, and every negative R^2 was
     silently reported as 0.0.

The Q-AI column was also generated by an untrained QuantumOrchORAgent whose
weights are randomly initialised, so it differed on every run.

Use q_ai_governance/benchmark_snapshot_real.py instead. It runs on the real
905-proposal / 6.24M-vote Snapshot record fetched by
q_ai_governance/fetch_snapshot_dataset.py, splits temporally so nothing is
predicted with hindsight, and reports R^2 as computed.

Retained only so the retracted numbers remain inspectable.
"""
